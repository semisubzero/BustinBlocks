...
-- Initialize the store and load products
	...
	function transactionCallback( event )
        ...
        elseif transaction.state == "failed" then
                ...
                native.showAlert("Failed",transaction.errorType,{"Darn"})
 
        else
                print("unknown event")
                native.showAlert("wot","Unknown event",{"Ok"})
        end
...
-- RandomThings
	...
	Price = display.newText("Price: "..Cost,40,410,nil,40)
		...
		elseif(StoreTile == 11)then
			...
			if(PurchasePressed == true)then
				if(store.canMakePurchases == true)then
					store.purchase({"com.hazardsoftware.BustinBlocks.BuyPoints"})
				else
